...
async function validateVersioning(validationResults) {
  ...
  validationResults
    ...
    .forEach(r => {
      ...
      const domain = interfaceName.split('-')[0];
  ...
  for (const [groupKey, interfaces] of Object.entries(interfaceGroups)) {
    // Group by interface name
    const nameGroups = {};
    interfaces.forEach(iface => {
      if (!nameGroups[iface.name]) {
        nameGroups[iface.name] = [];
      }
      nameGroups[iface.name].push(iface);
    });
    
    // Check each interface against previous versions
      ...
      for (let i = 1; i < versions.length; i++) {
        ...
        if (currMajor > prevMajor) {
          ...
          if (changes.hasBreakingChanges) {
            result.hasErrors = true;
            result.errors.push(`Minor version bump ${prevVersion.version} → ${currVersion.version} in ${name} contains breaking changes:`);
          ...
          if (changes.hasBreakingChanges) {
            result.hasErrors = true;
            result.errors.push(`Patch version bump ${prevVersion.version} → ${currVersion.version} in ${name} contains breaking changes:`);
            changes.breakingChanges.forEach(change => {
              result.errors.push(`  - ${change}`);
            });
          }
